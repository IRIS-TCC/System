<?php
    ...
    $apresentar = '';
            ...
            $sql = "SELECT (nomeMedico), (crmMedico), (nomePaciente)
             AS 'Nome', (rgPaciente), (dataAtestado), (diasAtestado) 
             FROM tbatestado INNER JOIN tbPaciente ON 
             (tbPaciente.codPaciente = tbatestado.codPaciente) INNER JOIN tbMedico ON 
             (tbMedico.codMedico = tbatestado.codMedico)
             WHERE tbpaciente.codPaciente = '".$codPaciente."' 
             AND tbatestado.dataAtestado = (SELECT MAX(dataAtestado)) AND tbmedico.codMedico = '".$codMedico."' LIMIT 1";
