...
//
//
public function cadastrarAnamneseOcorrencia($Ocorrencia,$codAnamnese){
    $conexao = Conexao::pegarConexao();
    $queryUpdate = "update tbProntuario
                set codAnamnese = '".$codAnamnese."'
                where codPaciente like '".$Ocorrencia->getCodPaciente()."'" ;
...
//
    public function listar($numCartaoSus){
        $conexao = Conexao::pegarConexao();
        $querySelect = "SELECT * FROM tbOcorrencia INNER JOIN tbpaciente on (tbOcorrencia.codPaciente = tbpaciente.codPaciente)
    ...
    public function listarOcorrenciaTriagem(){
        ...
        order by horaEntradaOcorrencia";
        $resultado = $conexao->query($querySelect);
        $lista = $resultado->fetchAll();
        return $lista;   
    }
